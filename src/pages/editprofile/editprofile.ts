...
export class EditprofilePage {
 ...
 pasienbaru = {
  ...
  ionViewWillLoad() {
    ...
    console.log('Selected', this.user.sCity)
    console.log('Selected', this.user.sDistrict)
    console.log('Selected', this.user.$key)
  }
  
  updatePatient(name,jk,tl,myDate,ns,almt,sCity,sDistrict,sKlh,kp,no,agama){
    this.fire.authState.take(1).subscribe(auth => {
      this.db.list(`pasienbaru/${auth.uid}`).update(this.user.$key, {
        name : name,
        jk : jk,
        tl : tl,
        myDate : myDate,
  ...
  showToast(position: string) {
    let toast = this.toastCtrl.create({
      message: 'Your Data Succesfully Update',
      duration: 2500,
      position: position
    });
  ...
  initializeCity(){
    ...
    setKlhValues(sDistrict) {
        ...
        console.log('Selected', sDistrict)
    }
